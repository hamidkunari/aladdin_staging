...
<head>
    <title>
        @if($unreadNotifications = user()->unreadNotifications()->count())
            ({{ $unreadNotifications }})
        @endif
        @yield('title') | {{ \Settings::get('site_name', 'Aladdin') }}
    </title>
    ...
    <link rel="shortcut icon" href="{{ asset('uploads/settings/logo2.png')}}" style="height: 20px; width: 20px;" type="image/png">
    <!-- CSRF Token -->
                ...
                <div class="col-md-2">
                    @auth
                        ...
                        @if ($vendor_role  && !user()->hasRole($vendor_role))
                            {!! '<a href="' . url('marketplace/store/enroll') . '" class="btn btn btn-block" style="background-color: #024C26; color:white;">'.trans('Marketplace::labels.store.become_a_seller').'</a>' !!}

                        @endif
                    @endauth
                
                    <nav class="list-group list-group-root well">
                        <a style="background-color: #071A10; color:white; font-family: poppins; border-radius:0px" onMouseOver="this.style.background='#B7950B'"
   onMouseOut="this.style.background='#071A10'" class="list-group-item {{ \Request::is('dashboard')?'active':'' }}"
                           href="{{ url('dashboard') }}">
                           <i class="fa fa-tachometer"></i> @lang('corals-marketplace-master::labels.partial.dashboard')
                        </a>
            ...
            @endif
            <div class="{{ (isset($hide_sidebar) && $hide_sidebar)?'col-md-12':'col-md-8' }}">
                @yield('content')
            </div>
        </div>
    </div>
